...
# Configure logging early (before importing other modules that might use it)
logging.basicConfig(
    level=logging.INFO,
...
try:
    ...
    env_path = os_module.path.join(script_dir, "key.env")
    # Try to load from script directory first, then current directory
    if os_module.path.exists(env_path):
        load_dotenv(env_path, override=True)
        logger.info(f"✅ Loaded environment variables from: {env_path}")
    elif os_module.path.exists("key.env"):
        load_dotenv("key.env", override=True)
        logger.info("✅ Loaded environment variables from: key.env (current directory)")
    else:
        logger.warning("⚠️ key.env file not found. Using system environment variables only.")
except (ImportError, FileNotFoundError) as e:
    logger.warning(f"⚠️ Could not load dotenv: {e}. Using system environment variables only.")
...
# Configure Gemini API key if available (will be reconfigured per-request for safety)
...
if gemini_key:
    ...
    logger.info(f"✅ Gemini API key loaded (length: {len(gemini_key)} chars)")
else:
    logger.warning("❌ GEMINI_API_KEY not found in environment")

if google_key:
    logger.info(f"✅ Google API key loaded (length: {len(google_key)} chars)")
else:
    logger.warning("❌ GOOGLE_API_KEY not found in environment")

if google_cse:
    logger.info(f"✅ Google CSE ID loaded: {google_cse}")
else:
...
# Persistent background asyncio loop to run all async/gRPC work in a single event loop.
# This avoids "Future ... attached to a different loop" errors by ensuring all
# async work (including grpc.aio/generative model calls) use the same loop.
...
_bg_thread = threading.Thread(target=_start_background_loop, args=(BACKGROUND_LOOP,), daemon=True)
_bg_thread.start()


def _shutdown_background_loop():
    try:
        # Attempt to run cleanup coroutines (if available)
        try:
            from scrappingAndFactcheck import cleanup_connections
            fut = asyncio.run_coroutine_threadsafe(cleanup_connections(), BACKGROUND_LOOP)
...
async def cross_verify_image_and_text(image_path, text_data, config):
    ...
    try:
        ...
        try:
            ...
            gemini_key = os.getenv('GEMINI_API_KEY')
            if not gemini_key:
                logger.warning("GEMINI_API_KEY not found, skipping combined synthesis")
                results['combined_synthesis'] = "API key not configured"
            else:
                genai.configure(api_key=gemini_key)
                model = genai.GenerativeModel('gemini-2.0-flash')
...
def upload_image_with_text():
    ...
    try:
        ...
        # Verify the saved image is a valid image file (catch corrupted or empty uploads early)
        try:
            with Image.open(image_path) as _img:
                _img.verify()  # verify checks for file integrity without fully decoding
            logger.info(f"✅ Image verified after save: {image_path}")
        except Exception as e:
            logger.error(f"❌ Uploaded image appears invalid or corrupted: {e}")
            # Remove the potentially corrupted file to avoid downstream confusion
            try:
                os.remove(image_path)
            except Exception:
                pass
            return jsonify({
                'error': 'Uploaded image is invalid or corrupted',
                'details': str(e),
                'status': 'failed'
            }), 400
        ...
        if not google_api_key:
            logger.warning("⚠️ GOOGLE_API_KEY not found - reverse image search will fail")
        if not google_cse_id:
            logger.warning("⚠️ GOOGLE_CSE_ID not found - Google Custom Search will fail")
        if not gemini_api_key:
            logger.warning("⚠️ GEMINI_API_KEY not found - AI analysis will fail")
        ...
        if text_data:
            ...
            future = asyncio.run_coroutine_threadsafe(
                cross_verify_image_and_text(image_path, text_data, config), BACKGROUND_LOOP
            )
            ...
            future = asyncio.run_coroutine_threadsafe(
                detect_image_misinformation(image_path, config), BACKGROUND_LOOP
            )
            results = future.result(timeout=120)
...
def upload_text_only():
    ...
    try:
        ...
        future = asyncio.run_coroutine_threadsafe(
            initialize_fact_checker(news_text=text_content, misinformation_domain=domain), BACKGROUND_LOOP
        )
        fact_check = future.result(timeout=120)
...
if __name__ == "__main__":
    ...
    print(f"   {'✓' if gemini else '✗'} GEMINI_API_KEY: {'LOADED' if gemini else 'MISSING'}" + (f" ({len(gemini)} chars)" if gemini else ""))
    print(f"   {'✓' if google else '✗'} GOOGLE_API_KEY: {'LOADED' if google else 'MISSING'}" + (f" ({len(google)} chars)" if google else ""))
    print(f"   {'✓' if cse else '✗'} GOOGLE_CSE_ID: {'LOADED' if cse else 'MISSING'}" + (f" ({cse})" if cse else ""))
    print(f"   {'✓' if os.getenv('SERPAPI_API_KEY') else '⚠'} SERPAPI_API_KEY: {'LOADED' if os.getenv('SERPAPI_API_KEY') else 'Optional'}")
    print(f"   {'✓' if os.getenv('IMGBB_API_KEY') else '⚠'} IMGBB_API_KEY: {'LOADED' if os.getenv('IMGBB_API_KEY') else 'Optional'}")
    
    if not (gemini and google and cse):
        print("\n⚠️  WARNING: Some required API keys are missing!")
        script_dir = os.path.dirname(os.path.abspath(__file__))
        env_file = os.path.join(script_dir, "key.env")
        print(f"   Expected key.env location: {env_file}")
        print(f"   File exists: {'YES' if os.path.exists(env_file) else 'NO'}")
        print(f"   Current working directory: {os.getcwd()}")
    else:
        print("\n✅ All required API keys are loaded successfully!")
    print("\n" + "="*70)
