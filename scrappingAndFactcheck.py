...
async def classify_news_type_with_gemini(news_text: str, misinformation_domain: str) -> Optional[str]:
    """
    Use Gemini AI to classify content as Evergreen or Real-time News.
    
    Returns:
        'Real-time News' or 'Evergreen News' or None if classification fails
    """
    try:
        gemini_key = os.getenv('GEMINI_API_KEY')
        if not gemini_key:
            logger.warning("GEMINI_API_KEY not available for news type classification")
            return None
        ...
        prompt = f"""Classify the following content as either "Evergreen News" or "Real-time News".

CONTENT: {news_text}
DOMAIN: {misinformation_domain}

DEFINITIONS:
- "Evergreen News": Timeless information, general facts, health advice, nutrition questions, scientific facts, 
  educational content, "how-to" guides, or questions about general knowledge. These don't have time-sensitive urgency.
  
- "Real-time News": Breaking news, current events, recent happenings, announcements, updates on ongoing situations, 
  news about specific recent incidents, or time-sensitive information.

Analyze the content and respond with ONLY one of these exact phrases:
- "Evergreen News"
- "Real-time News"
...
Do not include any explanation, just the classification."""
        ...
        # Parse the response
        if "evergreen" in result.lower():
            logger.info(f"🤖 Gemini classified as: Evergreen News")
            return "Evergreen News"
        elif "real-time" in result.lower() or "realtime" in result.lower():
            logger.info(f"🤖 Gemini classified as: Real-time News")
            return "Real-time News"
        else:
            logger.warning(f"🤖 Gemini returned unclear classification: {result}")
            return None
            
    except Exception as e:
        logger.error(f"Gemini news type classification failed: {e}")
        return None


async def determine_news_type(misinformation_domain: str, news_text: str = "") -> str:
    """
    Determine news type using Gemini AI first, then fallback to keyword-based heuristics.
    
    Args:
        misinformation_domain: Domain category (Health, Finance, General, etc.)
        news_text: The text/claim to classify
    
    Returns:
        'Real-time News' or 'Evergreen News'
    """
    # Step 1: Try Gemini classification first (if available)
    if news_text:
        gemini_result = await classify_news_type_with_gemini(news_text, misinformation_domain)
        if gemini_result:
            return gemini_result
        logger.info("Gemini classification unavailable or inconclusive, using keyword-based fallback")
    
    # Step 2: Fallback to keyword-based heuristics
    return determine_news_type_internal(misinformation_domain, news_text)
...
def determine_news_type_internal(misinformation_domain: str, news_text: str = "") -> str:
    ...
    This is a fallback method using keyword-based heuristics when Gemini is unavailable.
    ...
    # Text-based heuristics (analyze keywords in news_text FIRST - more reliable indicator)
    # This should run before domain-based defaults to catch breaking news in any domain
    if news_text:
        ...
        evergreen_keywords = ["always", "never", "fact", "study shows", "research", "permanently", "universally", 
                              "according to research", "studies show", "medical fact", "scientific fact",
                              "does", "do", "make", "makes", "cause", "causes", "affect", "affects", "is", "are",
                              "should", "can", "may", "might", "benefit", "benefits", "harmful", "healthy",
                              "unhealthy", "good for", "bad for", "nutrition", "diet", "food", "eat", "eating"]
        
        # Health/nutrition question patterns (these are typically evergreen)
        health_question_patterns = [
            "does.*make", "does.*cause", "does.*affect", "is.*good", "is.*bad", "is.*healthy",
            "can.*make", "will.*make", "should.*eat", "is.*safe", "does.*help",
            "makes you", "causes you", "good for", "bad for", "fat", "weight", "lose", "gain"
        ]
        
        # Keywords that suggest real-time content
        realtime_keywords = ["breaking", "just", "today", "now", "latest", "recent", "happened", "occurred",
                            "reports", "announced", "confirmed", "denied", "unveiled", "revealed", "occurrence"]
        ...
        # Check for health/nutrition question patterns (evergreen)
        health_pattern_score = sum(1 for pattern in health_question_patterns if re.search(pattern, news_lower))
        if health_pattern_score > 0:
            evergreen_score += health_pattern_score  # Boost evergreen score for health questions
            logger.info(f"Detected health/nutrition question pattern (score: +{health_pattern_score})")
        
        # If text strongly suggests real-time, prioritize that (even for Health domain)
        if realtime_score > evergreen_score and realtime_score > 0:
            logger.info(f"Text analysis suggests Real-time News (score: realtime={realtime_score} vs evergreen={evergreen_score})")
            return "Real-time News"
        
        # If text strongly suggests evergreen, use that
        if evergreen_score > realtime_score and evergreen_score > 0:
            logger.info(f"Text analysis suggests Evergreen News (score: evergreen={evergreen_score} vs realtime={realtime_score})")
            return "Evergreen News"
        
        # Special case: If it's a question about health/nutrition (even in General domain), default to Evergreen
        if health_pattern_score > 0:
            logger.info(f"Health/nutrition question detected in '{misinformation_domain}' domain - defaulting to Evergreen News")
            return "Evergreen News"
    
    # Domain-based defaults (applied only if text analysis is inconclusive)
    # Health domain often contains evergreen information (medical facts, health advice)
    if "health" in domain_lower:
        logger.info(f"Domain '{misinformation_domain}' detected as Health - defaulting to Evergreen News (text analysis was inconclusive)")
        return "Evergreen News"
...
async def initialize_fact_checker(news_text: str, misinformation_domain: str, news_type: Optional[str] = None, news_id: Optional[str] = None) -> FactCheckResult:
    ...
    try:
        # Auto-determine news_type if not provided (using Gemini AI first, then keyword fallback)
        if not news_type:
            news_type = await determine_news_type(misinformation_domain, news_text)
